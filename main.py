# =====================================================================
# main.py - VERSI FINAL (Absolute Path & APIRouter)
# =====================================================================
...
# --- BAGIAN 2: MODEL INPUT DARI FRONTEND ---
class PredictionInput(BaseModel):
    ...
    durasi_app1: str = Field(..., pattern=r"^\d{1,2}:\d{2}$", description="Format 'Jam:Menit', contoh: '1:30'")
    durasi_app2: str = Field(..., pattern=r"^\d{1,2}:\d{2}$")
    durasi_app3: str = Field(..., pattern=r"^\d{1,2}:\d{2}$")

# --- BAGIAN 3: INISIALISASI APLIKASI DAN ROUTER ---
app = FastAPI(title="API Prediksi Kualitas Tidur", version="8.0.0")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])
router = APIRouter(prefix="/api")

# --- BAGIAN 4: MEMUAT MODEL (DENGAN PERBAIKAN PATH) ---
try:
    # Menentukan path absolut berdasarkan lokasi file main.py
...
# --- BAGIAN 5: DEFINISI ENDPOINT ---

# Endpoint Root di aplikasi utama
@app.get("/", tags=["Root"])
def read_root():
    return {"message": "Welcome! API is running. Visit /api/docs for documentation."}

# Endpoint Prediksi menggunakan @router
...
def predict_sleep_quality(data: PredictionInput):
    ...
    # Tahap B: Persiapan data untuk model
    input_data = {
    ...
    # Tahap C: Prediksi & Format Output
    try:

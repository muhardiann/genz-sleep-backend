# =====================================================================
# main.py - VERSI FINAL (Lazy Loading & JSON Scaler)
# =====================================================================
...
import pandas as pd
import json # Menggantikan joblib untuk scaler
...
app = FastAPI(title="API Prediksi Kualitas Tidur", version="11.0.0")
...
def predict_sleep_quality(data: PredictionInput):
    ...
    # === TAHAP A: MEMUAT MODEL & PARAMETER SCALER ===
    try:
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        MODEL_PATH = os.path.join(BASE_DIR, 'HASIL_klasifikasi_model.h5')
        SCALER_PARAMS_PATH = os.path.join(BASE_DIR, 'scaler_params.json')
        ...
        print(f"❌ Gagal memuat model atau parameter scaler: {e}")
        return {"error": f"Gagal memuat file model/scaler di server. Detail: {e}"}

    # === TAHAP B: Pre-processing input ===
    ...
    # === TAHAP C: Penskalaan Manual & Prediksi ===
    try:
        # Melakukan penskalaan (scaling) secara manual dengan variabel yang benar
